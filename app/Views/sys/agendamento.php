...
<body>
    ...
    <style>
        ...
        </><script>const dataTableLangUrl="<?= base_url('assets/js/traducao-dataTable/pt_br.json'); ?>";
        ...
        function atualizarListaAlunosEdit() {
            ...
            if (alunosSelecionadosEdit.size===0) {
                ...
                alunosSelecionadosEdit.forEach((nome, matricula)=> {
                        const li=` <li class="d-flex justify-content-between align-items-center p-1" data-matricula="${matricula}" > <span>$ {
                            nome
                        }

                        </span> <button type="button" class="btn-close btn-close-white btn-sm remove-aluno-edit" ></button> </li>`;
 ...
 const turmasAlunosJson=JSON.stringify(row.alunos_por_turma).replace(/'/g, "&apos;");
                                ...
                                if (row.tipo==='turma') {
                                    ...
                                    data-turmas-alunos='${turmasAlunosJson}' ><u>$ {
                                        data
                                    }
        ...
        render: function(data, type, row) {
            ...
            return ` <div class="d-flex align-center justify-content-center gap-2" > <span data-bs-toggle="tooltip" title="Editar agendamento" > <button type="button" class="btn btn-inverse-success btn-icon me-1 btn-editar-agendamento d-flex align-items-center justify-content-center"
            data-bs-toggle="modal" data-bs-target="#modal-editar-agendamento"
            data-edit-info='${editInfoAttr}' > <i class="fa fa-edit" ></i> </button> </span> <span data-bs-toggle="tooltip" title="Excluir agendamento" > <button type="button" class="btn btn-inverse-danger btn-icon me-1 btn-excluir-agendamento d-flex align-items-center justify-content-center"
            data-bs-toggle="modal" data-bs-target="#modal-deletar-agendamento"
            data-nome="${row.turma_aluno}" data-delete-info='${deleteInfoAttr}' > <i class="fa fa-trash" ></i> </button> </span> </div> `;
        }
        }
        ...
        $('#listagem-agendamentos tbody').on('click', '.ver-alunos-link', function(e) {
                ...
                if (turmasAlunos) {
                    for (const nomeTurma in turmasAlunos) {
                        ul.append(`<li class="list-group-item turma-header" >$ {
                                nomeTurma
                            }

                            </li>`);

                        turmasAlunos[nomeTurma].forEach(nomeAluno=> ul.append(`<li class="list-group-item aluno-item" >$ {
                                    nomeAluno
                                }
                    ...
                    alunos.forEach(nome=> ul.append(`<li class="list-group-item" >$ {
                                nome
                            }
        ...
        if (document.getElementById('form-cadastrar-agendamento')) {
            ...
            $('#alunos_id').on('change', function() {
                    ...
                    if (valoresSelecionados.includes('select_all')) {
                        ...
                        const todosAlunos=$('#alunos_id option') .map(function() {
                                return this.value;
                            }) .get() .filter(v=> v !=='select_all');
        ...
        $(document).ready(function() {
                <?php if (session()->getFlashdata('sucesso')): ?> $.toast({
                    heading: 'Sucesso!',
            ...
            <?php endif; ?> <?php if (session()->getFlashdata('erros')): ?> $.toast({
                heading: 'Erro',
